...
class Spinner_game_5 {														// >176
	...
	constructor(totalTime, cards) {					
		...
		this.audioController= new AudioController();	//			178>
	}
...
function ready(){
	...
	let cardsO = Array.from(document.getElementsByClassName('card-outer'));
    let cardsI = Array.from(document.getElementsByClassName('card-inner'));
	console.log(cardsI);
    console.log(cardsO);
	let totalTime= document.getElementById('time-remaining');
	let game = new Spinner_Game_5(100, cards);
		...
		cards.forEach(card => {														// >18
			card.addEventListener('click', ()=> {
				console.log(card);
				game.flipCard(card);						//Will flip a card when defined				 82>
